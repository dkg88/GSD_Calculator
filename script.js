...
function toggleUnits(unit) {
  ...
  document.getElementById('imperial-button').classList.toggle('active', !isMetric);
  document.getElementById('metric-button').classList.toggle('active', isMetric);
}
...
updatePlaceholders();
document.getElementById('imperial-button').classList.add('active');
...
document.getElementById('flight-height-up').addEventListener('click', () => adjustValue('flight-height', isMetric ? 1 : 1));
document.getElementById('flight-height-down').addEventListener('click', () => adjustValue('flight-height', isMetric ? -1 : -1));
document.getElementById('desired-gsd-up').addEventListener('click', () => adjustValue('desired-gsd', isMetric ? 0.1 : 0.1));
document.getElementById('desired-gsd-down').addEventListener('click', () => adjustValue('desired-gsd', isMetric ? -0.1 : -0.1));

document.getElementById('desired-gsd').addEventListener('input', function (event) {
  const selectedIndex = document.getElementById('drone-select-button').innerText;
  const drone = dronesData.find(d => d.name === selectedIndex);

  if (drone && event.target.value) {
    const gsd = parseFloat(event.target.value);
    if (!isNaN(gsd)) {
